...
function parseBowlerLine(line) {
  ...
  if (nameEndIdx > 0) {
    ...
    // BLS prints "___" instead of a number when a bowler/sub hasn't bowled
    // enough games yet to have an established average — treat as unrated
    // (null) rather than defaulting to 0.
    const bookAvg = /^_+$/.test(lastPart) ? null : (parseInt(lastPart) || 0);
  ...
  // When BLS hasn't established an average yet it prints "___" in place of
  // the number — strip that trailing run of underscores before matching so
  // the row still parses (instead of failing to match and being dropped
  // entirely), and mark the average as unrated (null).
  const noAvgYet = /_+$/.test(line);
  const lineForMatch = noAvgYet ? line.replace(/_+$/, '') : line;

  const m = lineForMatch.match(/^([A-Za-z][A-Za-z\s\.\'\-\*]+?)(\d[\d\sa]*)$/);
  if (m) {
    const rawName = m[1].trim();
    if (noAvgYet) {
      return { rawName, bookAvg: null };
    }
...
export function parseRosterPDF(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Page-break junk: pdf-parse sometimes runs a page's footer straight into
    // the next page's header with no line break in between, e.g.
    // "...Classic Bowling Center9/9/2026  10:06 PM  Page 1 of 3" (filtered by
    // the BLS- check in isBowlerLine) immediately followed by its own line
    // "LGBT Wed Community 2026-279/9/2026    Week 1 of 29Page 2". The league
    // name prefix is configurable per league/season (e.g. "LGBT Wednesday
    // Community" vs "LGBT Wed Community"), so match on BLS's own fixed
    // "Week X of Y ... Page Z" boilerplate instead of a specific league name
    // — this generalizes across leagues and doesn't depend on which team's
    // roster happens to straddle a page break.
