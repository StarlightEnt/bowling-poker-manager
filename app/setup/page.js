...
const styles = {
  ...
  teamCard: {
    ...
    borderRadius: '8px',
    marginBottom: '10px',
    overflow: 'hidden',         // keeps rounded corners with table inside
  },
  teamHeader: {
    fontFamily: "'Bebas Neue', sans-serif",
    fontSize: '15px',
  ...
  // Table fills the card edge-to-edge (no outer padding)
  rosterTable: {
    width: '100%',
...
export default function SetupPage() {
  ...
  async function handleRosterUpload(file) {
    ...
    try {
      const res = await fetch('/api/setup/parse-roster', { method: 'POST', body: formData });
  ...
  async function handleScheduleUpload(file) {
    ...
    formData.append('pdf', file);
    try {
      const res = await fetch('/api/setup/parse-schedule', { method: 'POST', body: formData });
      const data = await res.json();
  ...
  async function handleSave() {
    if (!rosterData || !seasonName) return;
    setSaving(true);
    setSaveStatus({ type: 'loading', message: 'Saving season data...' });
    try {
      const res = await fetch('/api/setup/save-season', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ seasonName, teams: rosterData.teams, subs: rosterData.subs, weeks: scheduleData?.weeks || [] }),
      });
      const data = await res.json();
      if (data.error) { setSaveStatus({ type: 'error', message: data.error }); return; }
      setSaveStatus({ type: 'success', message: 'Season saved successfully! Ready to check in bowlers.' });
    } catch {
  ...
  // ── Shared table structure for both team and subs tables ──
    ...
    <div>
      ...
      {true && (
        <div style={styles.card}>
          ...
          <button style={saving || !seasonName || !scheduleData ? styles.btnDisabled : styles.btn} onClick={handleSave} disabled={saving || !seasonName || !scheduleData}>
      ...
      {/* Schedule Preview */}
      {scheduleData && (() => {
        // Find max number of positions across all weeks
