// PATH: app/[leagueSlug]/[seasonSlug]/roster/page.js
'use client';
...
const S = {
  ...
  rosterTable: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' },
  th: {
    background: 'var(--surface2)',
    color: 'var(--muted)',
    padding: '7px 12px',
...
function ImportPanel({ onImportComplete, leagueSlug, seasonSlug }) {
  ...
  async function handleFile(f) {
    ...
    try {
      ...
      const res = await fetch(`/api/${leagueSlug}/${seasonSlug}/roster/import?preview=true`, { method: 'POST', body: formData });
  ...
  async function handleCommit() {
    ...
    setCommitting(true);
    try {
      const res = await fetch(`/api/${leagueSlug}/${seasonSlug}/roster/import?preview=false`, {
        method: 'POST',
...
export default function RosterPage({ params }) {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const res = await fetch(`/api/${leagueSlug}/${seasonSlug}/roster`);
  ...
  return (
    <div>
      ...
      <ImportPanel onImportComplete={loadData} leagueSlug={leagueSlug} seasonSlug={seasonSlug} />
