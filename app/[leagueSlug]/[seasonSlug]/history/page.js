// PATH: app/[leagueSlug]/[seasonSlug]/history/page.js
'use client';
...
export default function HistoryPage({ params }) {
  ...
  useEffect(() => {
    ...
    const url = season === 'all'
      ? `/api/${leagueSlug}/${seasonSlug}/history`
      : `/api/${leagueSlug}/${seasonSlug}/history?season=${encodeURIComponent(season)}`;
    fetch(url)
      .then(r => r.json())
      .then(d => { setData(d); setLoading(false); })
      .catch(() => setLoading(false));
  }, [leagueSlug, seasonSlug, season]);
