// PATH: app/[leagueSlug]/[seasonSlug]/gamenight/page.js
'use client';
...
function DonationCard({ seasonId, weekNumber, initialDonation, isLocked, onRelockSuccess, leagueSlug, seasonSlug }) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/${leagueSlug}/${seasonSlug}/gamenight/donation`, {
        method: 'POST',
      ...
      if (isLocked) {
        const relock = await fetch(`/api/${leagueSlug}/${seasonSlug}/gamenight/donation/relock-charity`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
export default function GameNightPage({ params }) {
  ...
  async function loadData(week = null) {
    ...
    try {
      const url = week
        ? `/api/${leagueSlug}/${seasonSlug}/gamenight?week=${week}`
        : `/api/${leagueSlug}/${seasonSlug}/gamenight`;
  ...
  useEffect(() => { loadData(); }, [leagueSlug, seasonSlug]);

  async function handleSaveGame(gameData) {
    const res = await fetch(`/api/${leagueSlug}/${seasonSlug}/gamenight`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (result.success) {
      const url = `/api/${leagueSlug}/${seasonSlug}/gamenight?week=${data.week}`;
