...
export default function SettingsPage() {
  ...
  useEffect(() => {
    load();
    // Sync theme toggle from localStorage
    try {
  ...
  // Set today as default payout date when modal opens
  useEffect(() => {
    if (showPayoutModal) {
  ...
  async function confirmPayout() {
    ...
    setPayoutMsg('');
    try {
      const res = await fetch('/api/settings/charity-payout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
